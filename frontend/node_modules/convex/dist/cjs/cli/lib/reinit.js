...
async function reinit(ctx, deploymentType = "prod", config) {
  ...
  if (!projectSlug) {
    (0, import_context.logFailure)(ctx, "Aborted");
    return;
  }
...
`);
  ...
  const { projectConfig: projectConfigFromBackend } = await (0, import_config.pullConfig)(
    ctx,
    projectSlug,
    teamSlug,
    url,
  ...
  const mergedProjectConfig = await (0, import_config.mergeWithLocalConfig)(
    ctx,
  ...
  const functionsPath = (0, import_utils.functionsDir)((0, import_config.configName)(), projectConfigFromBackend);
