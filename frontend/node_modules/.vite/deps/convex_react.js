...
function jsonToConvexInternal(value, allowMapsAndSets) {
  ...
  if (entries.length === 1) {
    ...
    if (key === "$map") {
      ...
      for (const pair of value.$map) {
        ...
        const v2 = jsonToConvexInternal(pair[1], allowMapsAndSets);
        map.set(k, v2);
      }
      return map;
    }
  ...
  for (const [k, v2] of Object.entries(value)) {
    ...
    out[k] = jsonToConvexInternal(v2, allowMapsAndSets);
  }
  return out;
}
    ...
    get(_, prop) {
      if (typeof prop === "string") {
        ...
        if (pathParts.length < 2) {
          throw new Error(
            "API path must include at least 2 parts (module name and function name). Found: " + pathParts
          );
        }
...
var localFetch = globalThis.fetch;
...
function usePaginatedQuery(query, args, options) {
  ...
  let currState = state;
  if (getFunctionName(query) !== getFunctionName(state.query) || JSON.stringify(convexToJson(args)) !== JSON.stringify(convexToJson(state.args))) {
